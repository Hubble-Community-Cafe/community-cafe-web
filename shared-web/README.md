...
- **API client.** `getApiBaseUrl`, `fetchWithRetry`, and `getJson` resolve the backend URL from runtime config (`window.__RUNTIME_CONFIG__`, injected per container) with a build-time env fallback, and retry transient failures.
- **Shared types.** TypeScript interfaces for the five CMS modules plus media, mapped to the backend data model (`BarLocation`, `MenuItem`, `WeeklyHours`, `CafeEvent`, `BoardTerm`, ...).
